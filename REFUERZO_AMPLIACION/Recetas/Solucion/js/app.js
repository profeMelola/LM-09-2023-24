...
function renderizarRecetas(baseDeDatos) {
    recetas.innerHTML = ''; //si no hacen esto, se van añadiendo recetas una detrás de otra!!!!

    baseDeDatos.forEach(element => {

        const tarjeta = document.createElement("article");
        const tituloTarjeta = document.createElement("h2");
        tituloTarjeta.textContent = element.nombre;
        const imagenTarjeta = document.createElement("div");
        imagenTarjeta.classList.add("imagen");
        const img1 = document.createElement("img");
        img1.setAttribute("src", element.imagen);

        img1.addEventListener('click',()=>{
            document.querySelector('#info').textContent = element.descripcion;
        });
        

        imagenTarjeta.appendChild(img1);
        tarjeta.appendChild(tituloTarjeta);
        tarjeta.appendChild(imagenTarjeta);
        recetas.append(tarjeta);

        const otrainfo = document.createElement("div");
        const dificultad = document.createElement("p");
        dificultad.innerHTML = `<span>Dificultad: </span> ${element.dificultad}`;
        const comensales = document.createElement("p");
        comensales.innerHTML = `<span>Comensales:</span> ${element.comensales}`;
        const ingredientes = document.createElement("p");
        ingredientes.innerHTML = "<span>Ingredientes:</span>";
        ...
        otrainfo.append(dificultad);
        otrainfo.append(comensales);
        otrainfo.append(ingredientes);
        otrainfo.append(tabla);
        tarjeta.append(otrainfo);
...
function filtrarRecetas(evento){
    renderizarRecetas(baseDeDatos.filter(receta => receta.categoria == evento.target.textContent));
}
